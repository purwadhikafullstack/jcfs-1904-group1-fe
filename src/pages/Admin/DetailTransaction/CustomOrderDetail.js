...
function CustomOrderDetail() {
  ...
  const selected = productName.map((name) => {
  ...
  const fetchTransaction = async () => {
    try {
      const res = await axios.get(
        `transactions/details/custom/${transactionId}`
      );
      setInvoice(res.data.order[0].invoice);
      setImage(res.data.order[0].prescriptionPhoto);
      setUserId(res.data.order[0].user_id);
  ...
  const postCart = async () => {
    try {
      const data = { user_id: userId, selected };
      const res = await axios.post(`carts/details/custom`, data);
    } catch (error) {
      alert(error);
    }
  };
  const onAddToCartClick = () => {
    postCart();
    alert("Orders has been added to cart!");
  };
        ...
        <Box display="flex" margin="24px">
          <Box flex="1">
            ...
            <Box
              ...
              alignItems="center"
            >
              <FormControl sx={{ margin: "24px 0", width: 300 }}>
                <InputLabel id="demo-multiple-chip-label">Product</InputLabel>
                <Select
