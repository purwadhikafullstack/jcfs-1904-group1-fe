...
function DetailTransaction() {
  ...
  const fetchTransaction = async () => {
    try {
      ...
      setStatus(res.data.result[0][0].status);

      if (res.data.result[0][0].paymentPhoto) {
        setImage(res.data.result[0][0].paymentPhoto);
      }
    } catch (error) {
      alert(error);
    }
  ...
  if (status == "waiting payment") {
    ...
    button = (
      <Box display="flex" flexDirection="row">
        ...
        <Button onClick={RejectButton}>Reject</Button>
        <Button onClick={ReuploadPaymentButton}>Re-upload</Button>
      </Box>
    );
  ...
  return (
    <Box marginLeft="240px" display="flex" justifyContent="space-around">
      <Paper
        sx={{
          width: "70%",
      ...
      >
        <Box
          ...
          paddingTop="1px"
        >
          <Typography marginX="3px" marginY="auto" variant="h6">
            {invoice} Details
          </Typography>
          {status == "reject" ? (
            <Typography
        ...
        <Box>
          <Box display="flex" justifyContent="end">
            <Typography marginRight="24px">
              Total: {amount.toLocaleString("id")}{" "}
            </Typography>
          </Box>
